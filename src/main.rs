                ...
                let collector: Arc<dyn FlowCollector> = if config.conntrack_enabled {
                        Arc::new(ConntrackEnrichedCollector::new(
                                ...
                                Arc::new(CachedConntrackReader::new(
                                        ...
                                        Duration::from_secs(
                                                config.conntrack_refresh_interval_secs.max(1),
                                        ),
                                )),
                        ))
                } else {
                ...
                match collector.collect() {
                        Ok(mut batch) => {
                                ...
                                match observation.ingest_flows(&batch.flows) {
                                        Ok(ingested) => collector_health
                                                .record_batch(&batch.health, Ok(ingested)),
                                        Err(error) => {
                                                eprintln!(
...
mod tests {
        ...
        fn configured_test_app() -> Router {
                ...
                let auth =
                        Arc::new(AuthService::from_password("admin", "correct-password").unwrap());
                app(AppState {
                        observation,
                        auth,
                        collector_health: Arc::new(CollectorHealthTracker::new(None)),
