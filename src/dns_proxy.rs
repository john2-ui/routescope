...
impl DnsProxy {
        ...
        async fn handle_udp(
                ...
                upstream.connect(self.upstream_addr)
                        .await
        ...
        async fn handle_tcp(&self, mut client: TcpStream, peer: SocketAddr) -> Result<(), String> {
                loop {
                        ...
                        match client.read_exact(&mut length_bytes).await {
                                ...
                                Err(error) => {
                                        return Err(format!("read DNS TCP frame length: {error}"));
                                }
                        }
                        ...
                        client.write_u16(response_length).await.map_err(|error| {
                                format!("write DNS TCP response length: {error}")
                        })?;
        ...
        async fn forward_tcp(&self, request: &[u8]) -> Result<Vec<u8>, String> {
                ...
                upstream.write_u16(request_length)
                        .await
                ...
                let response_length = upstream.read_u16().await.map_err(|error| {
                        format!("read DNS TCP response length upstream: {error}")
                })?;
...
fn parse_dns_observations(
        ...
        for record in &response.answers {
                if let RData::A(address) = &record.data {
                        ...
                        ttl_secs =
                                Some(ttl_secs.map_or(record.ttl, |ttl: u32| ttl.min(record.ttl)));
                }
        }
...
mod tests {
        ...
        fn parses_ipv4_answers_into_dns_observations() {
                ...
                let observations = parse_dns_observations(
        ...
        fn ignores_ipv6_clients_and_responses_without_ipv4_answers() {
                let (request, response) = dns_packets();
                assert!(
                        parse_dns_observations("::1".parse().unwrap(), &request, &response, 1_000)
                                .is_empty()
                );
        ...
        async fn forwards_udp_query_and_queues_observation() {
                ...
                proxy.handle_udp(Arc::clone(&proxy_socket), client_addr, request)
                        .await
                        .unwrap();
        ...
        async fn forwards_tcp_query_with_dns_length_prefix() {
                ...
                let upstream_task = tokio::spawn(async move {
                        ...
                        stream.write_u16(u16::try_from(expected_response.len()).unwrap())
                                .await
                                .unwrap();
