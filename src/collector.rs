...
use crate::{
        ...
        domain::{
                ConnectionState, CounterReset, DomainAttribution, DomainConfidence, DomainSource,
                Flow, FlowDirection,
        },
};
use aya::{
                        ...
                        if let Association::Matched(entry) = snapshot.associate(flow) {
                                if let Some(mapping) = entry.nat_mapping() {
                                        ...
                                        flow.nat_destination_ip =
                                                Some(mapping.destination_ip.to_string());
...
impl FlowCollector for TcEbpfCollector {
        ...
        fn collect(&self) -> CollectionResult {
                ...
                let mut ebpf = self.ebpf.lock().map_err(|_| {
                        source_failure(self.source_name(), "eBPF state mutex is poisoned")
                })?;

                let map = ebpf.map_mut("flow_stats").ok_or_else(|| {
                        source_failure(self.source_name(), "flow_stats map is missing")
                })?;
                let mut stats: AyaHashMap<_, EbpfFlowKey, EbpfFlowValue> =
                        AyaHashMap::try_from(map).map_err(|error| {
                                source_failure(
                                        self.source_name(),
...
impl TcEbpfCollector {
        pub fn new(
                ...
                match tc::qdisc_add_clsact(&lan_interface) {
                        ...
                        Err(TcError::NetlinkError(error))
                                if error.raw_os_error() == Some(libc::EEXIST) => {}
                        Err(error) => {
                                return Err(source_failure(
                                        "tc-ebpf",
                                        format!(
                                                "failed to add clsact qdisc on {lan_interface}: {error}"
                                        ),
                                ));
                        }
                }
                ...
                {
                        let program =
                                ebpf.program_mut("routescope_tc_ingress").ok_or_else(|| {
                                        source_failure("tc-ebpf", "ingress program is missing")
                                })?;
                        let program: &mut SchedClassifier =
                                program.try_into().map_err(|error| {
                                        source_failure(
                                                "tc-ebpf",
                                                format!("invalid ingress program type: {error}"),
                                        )
                                })?;
                ...
                {
                        let program =
                                ebpf.program_mut("routescope_tc_egress").ok_or_else(|| {
                                        source_failure("tc-ebpf", "egress program is missing")
                                })?;
                        let program: &mut SchedClassifier =
                                program.try_into().map_err(|error| {
                                        source_failure(
                                                "tc-ebpf",
                                                format!("invalid egress program type: {error}"),
                                        )
                                })?;
                        program.load().map_err(|error| {
                                source_failure(
                                        "tc-ebpf",
                                        format!("failed to load egress program: {error}"),
                                )
                        })?;
                        program
        ...
        fn flow_from_entry(
                ...
                let first_seen =
                        monotonic_timestamp_ms(now_ns, value.first_seen_ns, observed_at_ms);
...
impl SimulatedCollector {
        ...
        fn with_start_time_and_interval(start_ms: i64, interval_secs: u64) -> Self {
                let step_ms = i64::try_from(interval_secs.max(1).saturating_mul(1_000))
                        .unwrap_or(i64::MAX);
...
fn monotonic_ns() -> Result<u64, String> {
        ...
        let seconds = u64::try_from(timespec.tv_sec)
                .map_err(|_| "monotonic seconds are negative".to_owned())?;
...
mod tests {
        ...
        fn conntrack_enrichment_adds_nat_and_state_without_changing_flow_id() {
                ...
                let snapshot = ConntrackSnapshot::from_entries(vec![conntrack_entry_for(
                        &first_batch.flows[0],
                )]);
