...
impl Config {
        ...
        pub fn from_env() -> Self {
                ...
                Self {
                        ...
                        aggregate_retention_days: env_u32(
                                "ROUTESCOPE_AGGREGATE_RETENTION_DAYS",
                                30,
                        ),
                        ...
                        simulator_enabled: std::env::var("ROUTESCOPE_ENABLE_SIMULATOR")
                                .map(|value| value == "1" || value.eq_ignore_ascii_case("true"))
                                .unwrap_or(false),
                        simulator_interval_secs: std::env::var(
                                "ROUTESCOPE_SIMULATOR_INTERVAL_SECS",
                        )
                        .ok()
                        .and_then(|value| value.parse().ok())
                        ...
                        dns_listen_addr: std::env::var("ROUTESCOPE_DNS_LISTEN_ADDR")
                                ...
                                .and_then(|value| value.parse().ok())
                                .unwrap_or_else(|| {
                                        "127.0.0.1:5353".parse().expect("valid default DNS address")
                                }),
