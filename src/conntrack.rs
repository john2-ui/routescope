...
impl ConntrackReader for NetlinkConntrackReader {
        fn snapshot(&self) -> Result<ConntrackSnapshot, String> {
                let connection = Conntrack::connect()
                        .map_err(|error| format!("connect to conntrack: {error}"))?;
...
fn state_from_kernel(flow: &KernelFlow) -> ConnectionState {
        ...
        {
                return match state {
                        TcpState::SynSent | TcpState::SynRecv | TcpState::SynSent2 => {
                                ConnectionState::New
                        }
                        ...
                        | TcpState::CloseWait
                        | TcpState::LastAck
                        | TcpState::TimeWait => ConnectionState::Closing,
...
mod tests {
        ...
        fn multiple_entries_for_same_tuple_are_ambiguous() {
                let snapshot = ConntrackSnapshot::from_entries(vec![
                        entry(40_000, 50_000),
                        entry(40_000, 50_001),
                ]);
