...
impl fmt::Display for AuthInitError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Storage(error) => write!(formatter, "authentication storage error: {error}"),
...
impl AuthService {
    ...
    pub fn from_repository(
        ...
        let account = if let Some((username, password_hash)) = repository.first_local_account()? {
            ...
            validate_password_hash(&password_hash)?;
            repository.insert_local_account_if_missing(&configured_username, &password_hash)?;
            Some(LocalAccount {
    ...
    pub fn from_password(username: &str, password: &str) -> Result<Self, AuthInitError> {
        ...
        let password_hash =
            Self::hash_password(password).map_err(|_| AuthInitError::InvalidPasswordHash)?;
...
pub(crate) fn cookie_value(headers: &HeaderMap, name: &str) -> Option<String> {
    headers
        ...
        .find_map(|(cookie_name, value)| {
            ...
            (cookie_name.trim() == name && is_token_safe(value)).then(|| value.to_owned())
        })
}
...
mod tests {
    ...
    fn repository_bootstraps_only_when_no_account_exists() {
        ...
        let auth =
            AuthService::from_repository(repository, "admin".to_owned(), Some(replacement_hash))
                .unwrap();
        assert!(auth.verify_credentials("admin", "first password"));
